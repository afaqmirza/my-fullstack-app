...
export default function PdfToPpt() {
  ...
  const uploadAndConvert = async (fileToProcess) => {
    ...
    try {
      const response = await executeTool('pdf-to-ppt', formData);
    ...
    <div className="pdf-to-ppt-page">
      <section className="tool-header-hero">
        ...
        >
          ...
          <p>Convert your PDF files to PowerPoint (PPTX) directly from your browser.</p>
        </motion.div>
      </section>
          ...
          {status === 'success' && (
            ...
            >
              ...
              <div style={{ marginTop: '2rem', display: 'flex', gap: '1rem', justifyContent: 'center' }}>
                <a 
                  href={downloadUrl} 
                  download={`${file?.name.substring(0, file?.name.lastIndexOf('.')) || 'converted'}.pptx`}
                  className="btn btn-primary"
                  style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', textDecoration: 'none' }}
                >
                  <FileText size={18} /> Download PPTX
                </a>
