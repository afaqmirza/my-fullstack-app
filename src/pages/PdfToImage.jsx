...
export default function PdfToImage() {
  ...
  const uploadAndConvert = async (fileToProcess) => {
    ...
    try {
      const response = await executeTool('pdf-to-image', formData);
  ...
  return (
    <div className="pdf-to-image-page">
      <section className="tool-header-hero">
        ...
        >
          ...
          <p>Convert your PDF pages to high-quality images (PNG) and download them as a ZIP file.</p>
        </motion.div>
      </section>
          ...
          {status === 'success' && (
            ...
            >
              ...
              <div style={{ marginTop: '2rem', display: 'flex', gap: '1rem', justifyContent: 'center' }}>
                <a 
                  href={downloadUrl} 
                  download={`${file?.name.substring(0, file?.name.lastIndexOf('.')) || 'converted'}_images.zip`}
                  className="btn btn-primary"
                  style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', textDecoration: 'none' }}
                >
                  <FileText size={18} /> Download ZIP
                </a>
